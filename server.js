...
/* send chat message command handler */
    ...
    socket.on('play_token', (payload) => {
        ...
        /* Make sure the current plau is coming from the expected player */
        if (
...
/* Return true if r + dr supoorts playing and c + dc */
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to make sure the adjacent support */
    if ((r + dr < 0 ) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0 ) || (c + dc > 7)) {
    ...
    /* Check the opposite color is present */
    if (board[r + dr][c + dc] !== other) {
        return false;
    }

    /* Check to make sure there is space for a matching color to capture token */
        ...
        if (sockets.size >= 1) {
            ...
            if ((games[game_id].player_white.socket != first) &&
                ...
                if (games[game_id].player_white.socket === "") {
                    ...
                    /* this player shoud be kicked out */
